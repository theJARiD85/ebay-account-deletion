...
export function createHandler({ fetchImpl = fetch } = {}) {
  return async ({ req, res, log = () => {}, error = () => {} }) => {
    ...
    try {
      ...
      const statusCode =
        caughtError instanceof RequestError ? caughtError.statusCode : 500;
      // Do not log request bodies, eBay identifiers, headers, OAuth material, or signatures.
      error(`[eBay account deletion] request failed with status ${statusCode}`);
      log("eBay account-deletion request ended without retaining notification data.");
